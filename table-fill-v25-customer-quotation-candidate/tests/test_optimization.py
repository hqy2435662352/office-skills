...
class OfficeCliReadRetryTests(unittest.TestCase):
    def test_outline_retries_once_after_empty_cold_start_failure(self):
        failed = mock.Mock(returncode=1, stdout=b"", stderr=b"")
        passed = mock.Mock(
            returncode=0,
            stdout=b'{"success": true, "data": {"sheets": []}}',
            stderr=b"",
        )
        with mock.patch.object(
            flatten_table.subprocess, "run", side_effect=[failed, passed]
        ) as run, mock.patch.object(flatten_table.time, "sleep") as sleep:
            result = flatten_table.officecli_outline("staged.xlsx")

        self.assertTrue(result["success"])
        self.assertEqual(run.call_count, 2)
        sleep.assert_called_once_with(1)

    def test_outline_stops_after_one_bounded_retry(self):
        failed = mock.Mock(returncode=1, stdout=b"", stderr=b"")
        with mock.patch.object(
            flatten_table.subprocess, "run", side_effect=[failed, failed]
        ) as run, mock.patch.object(flatten_table.time, "sleep"):
            with self.assertRaisesRegex(RuntimeError, "failed after retry"):
                flatten_table.officecli_outline("staged.xlsx")
...
class StyleGranularityTests(unittest.TestCase):
    """样式粒度决策事实 (issue 03): 占位行带样式/裸行检测 (XML 级) +
    digest 结论行 + 指纹不变. 决策事实入 digest/manifest, 不入结构指纹."""

    BARE_XML = ('<sheetData><row r="5"><c r="A5" s="0"/><c r="B5" s="0"/></row>'
                '<row r="6"/></sheetData>')
    STYLED_XML = '<sheetData><row r="7"><c r="A7" s="3"/></row></sheetData>'
    STYLES_XML = (
        '<styleSheet><cellXfs count="4">'
    ...
    def _bare_csv(self, n_rows: int = 20, content_rows: int = 3) -> Path:
        p = self.tmp / "t.csv"
        with open(p, "w", encoding="utf-8", newline="") as f:
            w = csv.writer(f)
            for orig in range(1, content_rows + 1):
                w.writerow(["标题" if orig == 1 else "值", orig])
            for orig in range(content_rows + 1, n_rows + 1):
                w.writerow([orig])
        return p

    def test_styled_style_indices_classification(self):
        from prepare_run import _styled_style_indices
        self.assertEqual(_styled_style_indices(self.STYLES_XML), {1, 2, 3})
        self.assertEqual(_styled_style_indices(None), set())

    def test_style_presence_bare_rows(self):
        from prepare_run import _style_presence
        self.assertEqual(_style_presence(self.BARE_XML, self.STYLES_XML, [5, 6]),
                         {"styled": False, "sample": None})

    def test_style_presence_styled_row(self):
        from prepare_run import _style_presence
        self.assertEqual(_style_presence(self.STYLED_XML, self.STYLES_XML, [7]),
                         {"styled": True, "sample": "A7"})

    def test_style_presence_without_styles_xml(self):
        """styles.xml 缺失 (或解析失败) → 一律视为裸行 (静默降级)."""
        from prepare_run import _style_presence
        self.assertEqual(_style_presence(self.STYLED_XML, None, [7]),
                         {"styled": False, "sample": None})

    def test_style_presence_missing_row(self):
        """XML 中不存在的行 (无单元格) → 裸行."""
        from prepare_run import _style_presence
        self.assertEqual(_style_presence(self.STYLED_XML, self.STYLES_XML, [8]),
                         {"styled": False, "sample": None})

    def test_detect_placeholder_rows_trailing_empty_run(self):
        from prepare_run import detect_placeholder_rows
        meta = {"dimensions": {"rows": 20}}
        csv_path = self._bare_csv()
        self.assertEqual(detect_placeholder_rows(meta, csv_path), list(range(4, 21)))
        # 末行有内容 → 空段 (行 1 永不进占位段)
        with open(csv_path, "a", encoding="utf-8", newline="") as f:
            csv.writer(f).writerow(["尾部", 21])
        meta2 = {"dimensions": {"rows": 21}}
        self.assertEqual(detect_placeholder_rows(meta2, csv_path), [])

    def test_collect_style_granularity_from_zip(self):
        """glue 全链路: 合成 xlsx zip (裸行占位 + 全零 xf) → 裸行结论;
        模板行 (表头) 无单元格 → 裸行."""
        import zipfile
        ...
        with zipfile.ZipFile(xlsx, "w") as z:
            ...
            z.writestr("xl/worksheets/sheet1.xml",
                       ...
                       '<sheetData><row r="5"><c r="A5"/></row><row r="6"/></sheetData></worksheet>')
            z.writestr("xl/styles.xml",
                       '<styleSheet><cellXfs count="1">'
                       '<xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/>'
                       '</cellXfs></styleSheet>')
        meta = {"sheet": "S",
                "dimensions": {"rows": 20, "cols": 2, "data_rows": 0},
                "header_band": {"header_rows": [2], "data_start_row": 3},
                "blocks": []}
        sg = collect_style_granularity(xlsx, meta, self._bare_csv())
        self.assertIsNotNone(sg)
        self.assertFalse(sg["placeholder"]["styled"])
        self.assertEqual((sg["placeholder"]["start"], sg["placeholder"]["end"]), (4, 20))
        # 表头行 2 在 XML 中无单元格 → 裸行
        self.assertFalse(sg["template_rows"]["header"]["styled"])

    def test_digest_renders_both_forms(self):
        """digest 结论行两种形态: 裸行 (占位自然下沉) vs 带样式 (样例坐标)."""
        import structure_digest
        base = {"sheet": "S", "file": "t.xlsx",
                "dimensions": {"rows": 20, "cols": 6, "formulas": 0, "errorCells": 0,
        ...
        csv = self.tmp / "d.csv"
        csv.touch()
        lines = "\n".join(structure_digest.build_digest(base, csv, None))
        self.assertIn("占位行样式: 裸行 (行 5-20)", lines)
        self.assertIn("模板行样式: header=行2 带样式", lines)
        base["style_granularity"] = {
            "placeholder": {"start": 7, "end": 10, "styled": True, "sample": "A7"},
            "template_rows": {"header": {"row": 2, "styled": True, "sample": "A2"}}}
        lines2 = "\n".join(structure_digest.build_digest(base, csv, None))
        self.assertIn("占位行样式: 带样式 (样例: A7, 行 7-10)", lines2)

    def test_fixtures_carry_both_forms(self):
        """probe fixture 两种形态: probe_workdir 裸行占位 / inplace_workdir
        带样式占位 (MXP 报价单形态)."""
        sg_bare = self.wd["manifest"]["style_granularity"]["target"]
        self.assertFalse(sg_bare["placeholder"]["styled"])
        self.assertEqual((sg_bare["placeholder"]["start"], sg_bare["placeholder"]["end"]),
                         (5, 20))
        with tempfile.TemporaryDirectory() as tmp2:
            wd2 = make_inplace_workdir(Path(tmp2))
        sg_styled = wd2["manifest"]["style_granularity"]["target"]
        self.assertTrue(sg_styled["placeholder"]["styled"])
        self.assertEqual(sg_styled["placeholder"]["sample"], "A7")

    def test_style_granularity_not_in_fingerprint(self):
        """决策事实不入结构指纹: 加/删 style_granularity 后指纹不变
        (旧 spec 不因 digest 增强失效, issue 03 验收)."""
        import json
        from prepare_run import facts_sha256, structure_facts
        meta = json.loads((self.tmp / "target_meta.json").read_text(encoding="utf-8"))
        self.assertIn("style_granularity", meta)
        with_sg = facts_sha256([structure_facts(meta)])
        meta.pop("style_granularity")
        self.assertEqual(facts_sha256([structure_facts(meta)]), with_sg)
...
class FillSpecContractTests(unittest.TestCase):
    ...
    # ── 01: append 块 remove_rows 越界 → REMOVE_TARGETS_APPEND_ZONE ──
...
class CapabilityMappingContractTests(unittest.TestCase):
    ...
    def test_per_group_total_trigger_minimal_mutation(self):
        """每组合计显式写法 (Q13): 最小变异实验 (issue 05) — 被拒 fixture
        的确切触发条件是聚合列进 nulls (锚点先清空后写公式, 特征 "first as
        empty"); 同形 spec 去掉该因素即编译通过 (埃及 11_FRESH本土 最终形态)."""
        wd = make_egypt_workdir(self.tmp)
        wd["workdir"] = self.tmp
        # 同形通过形态: 单块 + 显式范围聚合, 聚合列不进 nulls
        spec = spec_with(wd)
        spec["fingerprints"] = wd["manifest"]["fingerprints"]
        spec["mapping"]["targets"][0]["formulas"] = {
            "aggregates": [
        ...
        self.assertEqual(
            [op["path"] for op in plan["operations"]
             ...
             and "formula" in op.get("props", {})],
            ["/S/V7", "/S/V9", "/S/V11"])
        # 最小变异: 聚合列 V 进 nulls → 锚点双写 (first as empty)
        spec["mapping"]["targets"][0]["nulls"] = [
            {"col": "D", "rows": "all"}, {"col": "V", "rows": "all"}]
        codes = compile_fail_codes(wd, spec)
        ...
        try:
            compile_fill.compile_spec(spec, wd["manifest"], wd["workdir"])
            self.fail("mutated spec must be rejected")
        except SystemExit as e:
            self.assertEqual(e.code, 3)
        finally:
            sys.stderr = old
        self.assertIn("first as empty", buf.getvalue())
...
class CapabilitiesTests(unittest.TestCase):
    ...
    def test_capabilities_covers_contract_claims(self):
        ...
        for cid in ("group_merges_aggregate_same_col", "group_merges_aggregate_diff_col",
                    ...
                    "append_remove_rows_out_of_zone", "append_remove_rows_within_base",
...
class DocCoverageGuardTests(unittest.TestCase):
    ...
    def test_fillspec_error_code_table_has_append_zone(self):
        """REMOVE_TARGETS_APPEND_ZONE 必须在「常见编译错误速查」表内且带
        append-only 语义 (防章节误删/语义回退到无条件 inplace)."""
        table = self._error_code_table()
        self.assertIn("REMOVE_TARGETS_APPEND_ZONE", table)
        self.assertIn("append-only", table)
        self.assertIn("占位行自然下沉", table)
    ...
    def test_fillspec_q13_explicit_ranges_section(self):
        """契约章节含 Q13 小节 (单块显式范围聚合的接受边界 + nulls 触发
        条件 "first as empty")."""
        section = self._fillspec_section("组合行为契约")
        m = re.search(r"^### Q13:", section, re.MULTILINE)
        self.assertIsNotNone(m, "契约章节缺 Q13 小节")
        q13 = section[m.end():]
        self.assertIn("显式范围", q13)
        self.assertIn("nulls", q13)
        self.assertIn("first as empty", q13)
        self.assertIn("AGG_RANGE_INVALID", q13)
    ...
    def test_fillspec_layout_decision_tree_section(self):
        """「布局决策树」章节: 样式为第一判定条件, 三分支 + 缺陷码对照."""
        section = self._fillspec_section("布局决策树")
        self.assertIn("占位行样式", section)
        self.assertIn("裸行", section)
        self.assertIn("带样式", section)
        self.assertIn("inplace", section)
        self.assertIn("REMOVE_TARGETS_APPEND_ZONE", section)
        self.assertIn("TEMPLATE_ROW_GAP", section)

    def test_skill_md_mod_two_stage_loading(self):
        """SKILL.md MOD 规则两段加载: 提名给摘要, 裁决后加载全文,
        硬性要求 (进入上下文前必须已加载) 保留."""
        text = (SKILL_ROOT / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn("两段加载", text)
        self.assertIn("提名阶段", text)
        self.assertIn("不含完整规则集", text)
        self.assertIn("裁决后", text)
        self.assertIn("必须已加载", text)
    ...
    def test_skill_md_distrust_conversion_section(self):
        """SKILL.md「不信任事件转换」小节: 三件套 + 契约漂移为最高优先触发条件
        (防制度小节被误删或触发条件被降级)."""
        text = (SKILL_ROOT / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn("不信任事件转换", text)
        self.assertIn("三件套", text)
        self.assertIn("契约漂移", text)
        self.assertIn("最高优先", text)
        self.assertIn("contract test", text)
        self.assertIn("缺陷码", text)

    def test_known_traps_bare_placeholder_append_only(self):
        """KNOWN_TRAPS 沉淀裸行占位 → append-only 终态机械事实 (issue 03/04
        重放 oracle: 样式事实读 digest 结论行, 不 unzip sheet XML)."""
        text = (SKILL_ROOT / "references" / "KNOWN_TRAPS.md").read_text(encoding="utf-8")
        self.assertIn("裸行占位", text)
        self.assertIn("占位行样式", text)
        self.assertIn("自然下沉", text)
        self.assertIn("不写 remove_rows", text)
