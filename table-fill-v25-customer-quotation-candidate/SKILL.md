...
| 用户明确指定某 MOD, 且完整事实无冲突 | 直接采用, 记录 revision |
| 唯一候选, 信号全过, 无排除、无待决事实 | 自动采用 (status=resolved) |
| 多候选产生不同业务含义 | 暂停询问用户 |
| 单候选仍有未验证业务事实 | 视为歧义, 询问用户 |
...
**规则注入 (两段加载, 硬性)**: 候选 MOD 规则以两段加载进入 spec 撰写上下文 —
提名阶段只给摘要, 完整规则在用户裁决后加载 (缓解 mod_resolution 相位信息
过载: 埃及复盘 370s 的真实构成含 625 行提名输出, 2026-08-13):

1. **提名阶段 (mod_nominate 输出呈现时)**: 只呈现候选名 + 命中/待复验信号 +
   业务逻辑摘要 + 裁决选项 — **不含完整规则集**。
2. **裁决后加载**: 用户裁决选中 MOD 之后、写 FillSpec 之前, 才加载选中 MOD
   的完整规则 — 直接使用 `mod_resolution.json` 候选的 `rules` 字段 (或读
   MOD 文件全文)。映射关系、公式链、路由、字段继承、校验规则全部进入 spec
   撰写上下文:
   - spec 的 `columns`/`formulas`/`lookups`/`rows`/`decisions` 必须与 MOD
     规则一致 (如 FLD-006 原型机成本 = 源面价 − 铜管成本, FRM-001..008
     公式链);
   - 任何偏离必须记入 `decisions` 并写明理由;
   - **禁止在未加载 MOD 规则的情况下猜测映射关系与数据关系**;
   - 多候选 (ambiguous) 时, 裁决选项附带**各候选的规则证据摘要**, 完整
     规则在选定后加载 — 不用用户现场想;
   - `MOD NONE` 或 status=`none` (无候选) 时无规则注入, 按无 MOD 流程撰写。

**硬性要求不变**: 候选规则进入 spec 撰写上下文前**必须已加载** — 改变的
是加载时机 (裁决后) 与呈现粒度 (提名给摘要), 不是"是否加载"。
...
- **inplace 不变量 (13 码)**: 每目标至多一个 inplace 块且必须末位;
  占位区必须存在且坐标稳定 (append 区合法性 + 重叠检查); 前置 remove_rows 只在
  ≤ base_last_row (REMOVE_TARGETS_APPEND_ZONE 拒绝越界); sets 禁止进占位区;
  保留行残留按**每行自身原值**双基线检查
...
| 不可证明安全的操作 | **STOP**: 解释 + 推荐正确的领域能力 (如 REMOVE_TARGETS_APPEND_ZONE → 首选 append-only 合法终态, 占位行自然下沉; inplace 仅当占位行携带样式时才是条件选项) |
