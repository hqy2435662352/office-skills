...
def build_digest(meta: dict, csv_path: Path, candidates: dict | None) -> list[str]:
    ...
    # 样式粒度决策事实 (issue 03): 布局决策第一判定条件 (FILLSPEC「布局决策树」)
    sg = meta.get("style_granularity") or {}
    ph = sg.get("placeholder") or {}
    if ph.get("start") is not None and ph.get("end") is not None:
        if ph.get("styled") and ph.get("sample"):
            lines.append(f"- 占位行样式: 带样式 (样例: {ph['sample']}, 行 "
                         f"{ph['start']}-{ph['end']}) — inplace 成立 (占位区消费)")
        else:
            lines.append(f"- 占位行样式: 裸行 (行 {ph['start']}-{ph['end']}) — "
                         "clone-append, 占位行自然下沉 (append-only 合法终态)")
    tr = sg.get("template_rows") or {}
    parts = []
    for role, info in tr.items():
        if not isinstance(info, dict) or not info.get("row"):
            continue
        sl = "带样式" if info.get("styled") else "裸行"
        parts.append(f"{role}=行{info['row']} {sl}")
    if parts:
        lines.append("- 模板行样式: " + " | ".join(parts))
