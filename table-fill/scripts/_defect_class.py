...
def standard_fix(cls: str) -> str:
    fixes = {
        ...
        "row_anchor_missing": "行号空洞: 目标 sheet row 元素 r 值不连续, officecli `add after/from /row[N]` 锚点不存在。默认已由 workspace_init --init 在 staging 内一次修好(先于哈希/展平/指纹); 仍出现说明用了 --no-repair 或源==暂存被拒(row_gaps_deferred) → 独立 CLI: scripts/repair_row_gaps.py --input <xlsx> --all [--out <snapshot>] 产出新快照 → 重新 --init(哈希/展平/指纹按新字节重算) → 重编译 → 重执行 (repair 不做任何同步/patch)",
        "CLONE_STYLE_FIDELITY_MISMATCH": "新增数据行的列级样式剖面 != 块内锚点剖面: data 克隆源是「合并非锚点但不带块内格式」的行, 新块的类别列/聚合合并区会丢字体/填充/边框而 validate·issues·readback·render 全绿 → 换 corrective_action 列出的候选行(非锚点 且 样式剖面==众数), 重编译重执行",
