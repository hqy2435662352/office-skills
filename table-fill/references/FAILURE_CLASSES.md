...
| `row_anchor_missing` | officecli 报 `Anchor row N not found` | 行号空洞: 目标 sheet row 元素 r 值不连续, `add after/from /row[N]` 锚点不存在。**默认已由 `workspace_init --init` 在 staging 内一次修好** (先于哈希/展平/指纹) — 仍出现说明用了 `--no-repair` 或源==暂存被拒 (`row_gaps_deferred`) → `scripts/repair_row_gaps.py --input <xlsx> --all [--out <snapshot>]` 产出新快照 → **重新 `--init`** (哈希/展平/指纹按新字节重算; repair 不做任何同步/patch) → 重编译 → 重执行 | 埃及模板 1..21, 23..52 缺 22 |
