...
| **费用列空串进公式链 → 净价兜底 0 (2026-08-18)** | 用户打开交付物发现净价列显示全 0$，而机器证据（readback/issue/validate）全绿 | 费用列「源无此角色」写成**空字符串**（nulls 清空）；空串被净价公式 `J-K-L-M-N` 求值链按 **非空文本** 判错误 → `#VALUE!` → `IFERROR(...,0)` **兜底成 0**（Case 07 §8：既有模板费用列是数值 0 因此无此问题） | **0-口径二分**：入公式链的纯数据/非公式字段（财务费用/OA 信保/返点/其他费用）无数值 → 写数值 `0`（列映射 `value: "0"` 常量），**不写空串**；仅独立展示、不入任何公式链的字段才可留空（见 FILLSPEC Q5） |
| **`[1]` 前缀 = 外部工作簿引用 (2026-08-18)** | `_xlfn.XLOOKUP(C,[1]阿尔及利亚!…)` 的 `[1]` 是**外部工作簿**引用句柄（`[1]`=首个外部链接）；参考行带模板缓存值可**显示旧值**，但**新写公式未重建外部链接不可求值**（`formula_not_evaluated`，execute/readback 落空） | 编译器/执行器无法解析跨工作簿外部链接；常规复制公式会把外部引用句柄带进新数据行 | 该列**不含本管/外部数据 → 直接 `null`**（克隆残留置入 nulls），**不常规复制公式**；需要值则从 digest/索引物化（见 FILLSPEC Q5/Q15）。`[1]` 外部引用列不复刻公式（Case 08 U7；骨架见 combination_patterns.yaml `single_quotation_block_append`） |
...
| **索引被手工改写 → 静默全空 (2026-08-13)** | 清洗脚本重写 inheritance.json 丢 `field_consensus` → normalize 后索引表为空 → D/F/X 全部静默留空 (missing: empty), 编译照常产出, 唯一暴露点是 Agent 审 mapping.md 发现 Written values 全空 | normalize_lookup_data 只认 field_consensus; 手改 JSON 丢字段 | 编译器现以 LOOKUP_TABLE_EMPTY (exit 3) 编译拒绝; 整列未命中 (索引非空) → LOOKUP_COLUMN_ALL_MISSING 警告 (审查呈现) — 索引清洗一律用 `build_inheritance_index.py` 重建, **禁止手改 JSON** |
| **索引自引用: 目标 sheet 作输入 → 静默缺失 (2026-08-13)** | 拖多12K (Z2U20102005764) 的 factory_model 出现两候选: 09_Fresh拖多 = `KFR-32G/YXDBp(E)(005764)`, 11_FRESH本土 既有块 = `KFR-32G/YXDBp(E)(011209)` → 共识 conflict → 按缺失处理 → 静默留空 | 把本次填充的**目标 sheet** (11_FRESH本土) 也喂进了 `build_inheritance_index.py` — 目标 sheet 既有数据是历史输出, 不是字段权威源, 与独立数据 sheet 同 SKU 多值 | 构建索引**只喂独立数据 sheet** (如 01_埃及机型、09_Fresh拖多), 排除目标 sheet; 整列未命中的 LOOKUP_COLUMN_ALL_MISSING 警告 corrective_action 已含自引用排查提示 (重建索引前先查输入 sheet 清单) ; 仅为 lookup/inheritance 的 sheet 不 flatten 进 fill manifest, 直接用 --input/--sheet 建索引 (使用角色契约见 FILLSPEC「Fill source use vs lookup-only use」) |
...
| **compile 后修改 staged 文件 (2026-08-13)** | execute 报 `INPUT_HASH_DRIFT` (exit 3, 在 copy_template 之前), 或 deliver 报 HASH_DRIFT (输入三方核对) — 从未产生 "哈希齐全但输入漂移" 的 Draft | staged 输入 = 本次填充的输入快照; compile 后手工改动 = 静默改填充语义 (旧版 execute 只抄 manifest 的 outline 期 files[].sha256, 不重算, 盲区) | 机械事实: 输入哈希在 compile (plan.input_hashes 重绑定) / execute (copy 前重算比对) / deliver (三方核对) 三个边界重算 — 改 staged 文件 → 恢复原文件, 或重跑 prepare_run + compile_fill.py 重绑定。**谁能合法改 staged**: 只有 `workspace_init --init` 自己 — 它在 staging 内修行号空洞, 且发生在记录任何哈希**之前**, 所以修复后字节 == manifest 哈希; 独立 `repair_row_gaps.py` **不碰 staged**, 它只产出新快照并要求重新 init。compile 之后再改 staged 一律视为漂移 (fail-closed) |
...
| **MOD_INDEX 缺 `## Registered MODs` 标题 → 解析静默为空 (2026-08-17)** | 提名/守卫测试用无标题的注册表 → `parse_mod_index` 恒返回 [] → 候选为空或 IndexError, 或误判 "no MOD registered" | 解析器统一到 `_mod_catalog.parse_mod_index` 后要求该小节标题 (`_SECTION_HEADING_RE`); 统一前的手写 fixture 无标题 | 注册表一律带 `## Registered MODs` 小节标题再写表; 手写示例文档勿漏（示例均已按此修正） |
| **block 顶层聚合键静默丢弃 (2026-08-19)** | block 顶层写 `aggregates:`/`per_row:`/`group_aggregates:` 或 typo 键 (单数 `formula`/`column`) → 编译照常通过, plan 无对应操作 (静默丢弃; Case 05 反推 3 次编译才发现) | `resolve_blocks` 用 `dict(b)` 透传全部块键, `_emit_block_ops` 只读 `cfg.get("formulas", {})` — 块顶层聚合键无 lowering 也不报错 | 编译器现以 `BLOCK_KEY_STRUCTURE_INVALID` (exit 3) 编译期拒绝, corrective_action 点名应写 `formulas: {aggregates: [...]}` / `formulas: {per_row: ...}` 等正确嵌套; 块级合法顶层键 = clone_roles/rows/columns/formulas/merges/group_merges/nulls/remove_rows/styles |
...
| 陷阱 | 症状 | 根因 | 修复 |
|------|------|------|------|
| **execute crash window (v3)** | 中断后凭 draft 存在性直接交付 — 但 execute 在写 draft 与写 receipt 之间崩溃, 验证证据链不完整 | draft 存在但 receipt 缺失/不匹配; 只查产物存在性不查证据对 | 交付前 `promote_output.py` 三方哈希核对：draft+receipt 均有效才可交付; draft 存在但 receipt 缺失/不匹配 → **重跑 execute**（用已有 execution_plan.json） |
| **status ≠ truth (v3)** | task_status.json 说 compiled/delivered, 但产物缺失或 hash 漂移, 跳过必要阶段 → 坏输入进 compile/execute | 状态文件是生命周期**索引**, 不是真值源; 中断/半写都可能让状态领先或落后于产物事实 | 断点一律以 artifact 存在性 + hash 校验判定; task_status 只作索引与展示（task_status.json 由脚本单一写者维护） |
| **Office 并发竞争** | 并发 flatten/execute 时 validate_state=fail 或文件锁错误；execute 并发窗口曾出现 readback 假失败（文件实际有值、读回为空，需人工 `officecli get` 复核） | Windows resident 进程持文件锁; Office 并发上限是**环境经验**, 不是架构契约 | v3 收敛后调度器退役，单进程顺序执行，execute 天然串行; execute 失败自动复核 (flush → get 读回计划目标格 → 有值即 recovered/无值 retry 一次 → 确认失败, 详见 LAYER4_EXECUTE_LOOP.md §execute 失败复核); 换 Office 发行版先按实证复核再调 |
| **输入事实改变后继续用旧 run (v3)** | 输入事实已变（source/template/sheet 引用、源 hash 漂移、模板重建、MOD 裁决变化），还在旧 run 上修 spec 重编译 → 交付基于已失效输入 | 失败二分未执行: 输入事实改变时未重新初始化, 而是继续修补旧版本 | 失败二分: **输入事实未变**（execute 后修 fill_spec 迭代）→ 修 fill_spec → 重编译 → 重执行 → 重交付（版本历史 = 文件版本命名, 无迭代/替代抽象）；**输入事实改变** → 删除 derived 文件后重新 --init/--prepare |
| **失败就重跑 vs 修 spec (T05, v3)** | 普通失败/语义修复误入「重新初始化」: 修 fill_spec 后直接删 derived 重 --init → 重复 prepare flatten, 浪费 | 未区分「修 fill_spec 重跑」（输入事实未变）与「重新初始化」（输入事实已变） | 调用策略: 输入事实未变 → execute 失败由自动复核 + retry (flush → get 复核目标格) 兜底、语义缺陷修 fill_spec 后重编译重执行；仅输入事实改变才删除 derived 文件重新 --init/--prepare |
...
| **lookup 索引完整性 (2026-08-13)** | 索引归一化后为空 (0 entries — 手改 JSON 丢 field_consensus) → 编译器 LOOKUP_TABLE_EMPTY (exit 3) 拒绝, **不再静默全空**; 索引非空但声明 lookup 列全部未命中 → LOOKUP_COLUMN_ALL_MISSING 警告 (可继续编译, 审查呈现) — 索引清洗一律用 build_inheritance_index.py 重建, **禁止手改 JSON** (见 FILLSPEC Q15) |
...
| 陷阱 | 症状 | 根因 | 修复 |
|------|------|------|------|
| **模板行号空洞 (row gap)** | 执行报 `Anchor row N not found in <sheet>`; outline/digest 报 51 行但 row 元素 r 值不连续 (如 1..21, 23..52, 缺 22) | 模板 XML 缺失某 r 值 row 元素 (历史删除行残留), 空洞可能在**用户原始文件**里就存在 | **默认已自愈**: `workspace_init --init` 在 staging 内检测 + 物化缺失行 (先于哈希/展平/指纹), 一趟出事实空间。仍出现该行说明 init 拒绝或跳过了修复 (`--no-repair`, 或源==暂存被拒 → `row_gaps_deferred` 带 `reason`) → 走独立 CLI `repair_row_gaps.py --input <xlsx> --all [--out <snapshot>]` 产出新快照 → **重新 `--init`**。已实证: `add after` 无法闭合空洞 (插入行落在空洞后的 r 值, 锚点链永久断裂); style-only 写 (`set /Sheet/A<r> numberformat=0.00`) 可物化空行元素不留内容, `value:null` 只在 officecli 保存过的文件上有效; **set 后必须 `officecli close` 刷盘** (resident 延迟写未落盘时紧跟的重读复见空洞, 2026-08-13 实测; CLI 与 init 都已内置 flush + 逐 sheet 复核) |
| **行洞修复 = 指纹必然变化 (2026-08-13)** | 修复后不重算就重编译 → FILLSPEC_FINGERPRINT_MISMATCH; 手工重 flatten + 抄指纹 → 每步都可能抄错/漏跑 (埃及 FRESH 曾发生 manifest 被再次 flatten 覆盖 target 指针的小岔) | 机械事实: 行洞修复 = 输入字节修改 = 指纹**必然变化** (如 dims.rows 3→4, 入结构指纹) | 不要试图"同步指纹": 修复脚本**不重跑 flatten、不 patch spec** (ADR 0021 收缩; `--workdir`/`--patch-spec` 现被 `LEGACY_CLI_REJECTED` 显式拒绝)。正确终点是让修复后的字节**重新进入 canonical init** — 修复发生在 init 内部时 (默认路径) 哈希/展平/指纹本就按修复后字节一次算好; 用独立 CLI 时则以新快照重新 `--init`。局部同步=在已推进的事实空间上做半改, 已被显式排除 |
