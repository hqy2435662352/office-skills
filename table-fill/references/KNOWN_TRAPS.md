...
| **compile 后修改 staged 文件 (2026-08-13)** | execute 报 `INPUT_HASH_DRIFT` (exit 3, 在 copy_template 之前), 或 promote 报 HASH_DRIFT (输入三方核对) — 从未产生 "哈希齐全但输入漂移" 的 Draft | staged 输入 = 本次填充的输入快照; compile 后手工改动 = 静默改填充语义 (旧版 execute 只抄 manifest 的 outline 期 files[].sha256, 不重算, 盲区) | 机械事实: 输入哈希在 compile (plan.input_hashes 重绑定) / execute (copy 前重算比对) / promote (三方核对) 三个边界重算 — 改 staged 文件 → 恢复原文件, 或重跑 prepare_run + compile_fill.py 重绑定; 唯一合法修改者是 repair_row_gaps.py (其重 flatten 只同步指纹, files[].sha256 会过期 — 重编译即重绑定) |
