...
<!--
  ...
  输出提名卡, 由用户裁决 (选择或 NONE)。结构类信号中 dimension_set 按 digest
  表头角色事实核对 (digest 已喂时给出真 hit/miss, 未喂时 pending);
...
| **Scope Signals (+)** | yes | 表事实信号 (提名阶段可验证的: semantic_type/target_title/source_pattern/target_pattern/product_domain/sheet_marker/dimension_set(digest 已喂时); 待L2复验的: measure_set/formula_chain/block_layout/unit_convention/time_granularity). 格式 `signal_kind::value`. |
...
| `dimension_set` | ✅ (digest 已喂时, 按表头角色事实核对; 未喂 → pending) | digest 表头角色 (如 `product_sku` → Z码/SKU/型号/货号) |
