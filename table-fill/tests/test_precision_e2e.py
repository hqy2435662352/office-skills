...
class PrecisionExecutionContractTests(unittest.TestCase):
    ...
    def test_tampered_staged_template_rejected_before_fill(self):
        """Issue 03 最小篡改复现: compile 后修改 staged template →
        execute 在 copy_template 之前以 INPUT_HASH_DRIFT 拒绝 (exit 3),
        不产出 validated_draft, 绝不产生 'hash-complete but input-drifted' Draft."""
        proc = self._compile({"transform": "round4"})
        self.assertEqual(proc.returncode, 0, proc.stdout[-800:] + proc.stderr[-800:])
        with open(self.workdir / "template.xlsx", "ab") as f:
            f.write(b"TAMPERED-AFTER-COMPILE")
        proc = self._execute()
        self.assertEqual(proc.returncode, 3, proc.stdout[-800:] + proc.stderr[-800:])
        self.assertIn("INPUT_HASH_DRIFT", proc.stderr)
        failure = json.loads(
            (self.workdir / "_draft_failure.json").read_text(encoding="utf-8"))
        self.assertEqual(failure["code"], "INPUT_HASH_DRIFT")
        self.assertIn("template.xlsx", failure["drifted_inputs"])
        self.assertIn("corrective_action", failure)
        self.assertFalse((self.workdir / "validated_draft.xlsx").exists())

    def test_tampered_staged_source_rejected_before_fill(self):
        """同样复现于 staged SOURCE: compile 后篡改 → execute 拒绝, 无 draft."""
        proc = self._compile({"transform": "round4"})
        self.assertEqual(proc.returncode, 0, proc.stdout[-800:] + proc.stderr[-800:])
        with open(self.workdir / "source.xlsx", "ab") as f:
            f.write(b"TAMPERED-AFTER-COMPILE")
        proc = self._execute()
        self.assertEqual(proc.returncode, 3, proc.stdout[-800:] + proc.stderr[-800:])
        self.assertIn("INPUT_HASH_DRIFT", proc.stderr)
        failure = json.loads(
            (self.workdir / "_draft_failure.json").read_text(encoding="utf-8"))
        self.assertEqual(failure["code"], "INPUT_HASH_DRIFT")
        self.assertIn("source.xlsx", failure["drifted_inputs"])
        self.assertFalse((self.workdir / "validated_draft.xlsx").exists())
