...
def main() -> None:
    ...
    # Input hash check — the plan's compile-time binding (the plan itself is
    # hash-verified via the trio above) vs the receipt's execution-time
    # recompute vs the CURRENT staged files. Any drift → HASH_DRIFT.
    # Fail-closed: missing evidence (no plan binding / no receipt record)
    # means the staged inputs cannot be verified → reject.
    plan_data = {}
    try:
